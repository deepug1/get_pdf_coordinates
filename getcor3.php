...
<body>
    <div class="container">
        ...
        <div id="document-dimensions">
            Document Size: <span id="doc-width">0</span>mm × <span id="doc-height">0</span>mm
            <div id="axis-dimensions">
                <span>Width: <span class="dimension-label" id="width-label">0mm</span></span>
                <span>Height: <span class="dimension-label" id="height-label">0mm</span></span>
            </div>
        </div>
    ...
    <script>
        ...
        let docDimensions = document.getElementById('document-dimensions');
        let docWidthSpan = document.getElementById('doc-width');
        let docHeightSpan = document.getElementById('doc-height');
        ...
        function renderPage(num) {
            ...
            pdfDoc.getPage(num).then(function(page) {
                ...
                // Get actual PDF dimensions in points (1/72 inch)
                let viewportUnscaled = page.getViewport({scale: 1.0});
                
                // Convert PDF points to mm (1 point = 0.352778 mm)
                pdfWidth = Math.round(viewportUnscaled.width * 0.352778 * 10) / 10;
                pdfHeight = Math.round(viewportUnscaled.height * 0.352778 * 10) / 10;
                
                // Update dimension display
                docWidthSpan.textContent = pdfWidth;
                docHeightSpan.textContent = pdfHeight;
                widthLabel.textContent = pdfWidth + 'mm';
                heightLabel.textContent = pdfHeight + 'mm';
                docDimensions.style.display = 'block';
                
                // Draw automatic measurement lines
                drawAutoMeasurements();
                ...
                // After rendering PDF, draw the grid and measurements
        ...
        // Draw automatic width and height measurement lines
        function drawAutoMeasurements() {
            ...
            measureCtx.moveTo(0, 20);
            measureCtx.lineTo(measurementCanvas.width, 20);
            measureCtx.stroke();
            
            // Add arrows to width line
            drawArrow(measureCtx, 0, 20, 10, 20, 5);
            drawArrow(measureCtx, measurementCanvas.width, 20, measurementCanvas.width - 10, 20, 5);
            
            // Draw width text
            measureCtx.fillStyle = 'white';
            measureCtx.font = 'bold 14px Arial';
            let widthText = `Width: ${pdfWidth}mm`;
            let textWidth = measureCtx.measureText(widthText).width;
            let textX = (measurementCanvas.width - textWidth) / 2;
            
            // Background for text
            measureCtx.fillStyle = 'rgba(0, 0, 255, 0.7)';
            measureCtx.fillRect(textX - 5, 10, textWidth + 10, 20);
            
            measureCtx.fillStyle = 'white';
            measureCtx.fillText(widthText, textX, 25);
            
            // Draw height measurement (left side)
            measureCtx.beginPath();
            measureCtx.strokeStyle = 'green';
            measureCtx.lineWidth = 2;
            measureCtx.moveTo(20, 0);
            measureCtx.lineTo(20, measurementCanvas.height);
            measureCtx.stroke();
            
            // Add arrows to height line
            drawArrow(measureCtx, 20, 0, 20, 10, 5);
            drawArrow(measureCtx, 20, measurementCanvas.height, 20, measurementCanvas.height - 10, 5);
            
            // Draw height text
            measureCtx.save();
            measureCtx.translate(30, measurementCanvas.height / 2);
            measureCtx.rotate(-Math.PI/2);
            
            let heightText = `Height: ${pdfHeight}mm`;
            textWidth = measureCtx.measureText(heightText).width;
            
            // Background for text
            measureCtx.fillStyle = 'rgba(0, 128, 0, 0.7)';
            measureCtx.fillRect(-textWidth/2 - 5, -10, textWidth + 10, 20);
            
            measureCtx.fillStyle = 'white';
            measureCtx.fillText(heightText, -textWidth/2, 5);
            measureCtx.restore();
        }
        
        // Function to draw arrow heads
        function drawArrow(ctx, fromX, fromY, toX, toY, headSize) {
            const angle = Math.atan2(toY - fromY, toX - fromX);
            
            ctx.beginPath();
            ctx.moveTo(fromX, fromY);
            ctx.lineTo(fromX + headSize * Math.cos(angle - Math.PI/6), fromY + headSize * Math.sin(angle - Math.PI/6));
            ctx.lineTo(fromX + headSize * Math.cos(angle + Math.PI/6), fromY + headSize * Math.sin(angle + Math.PI/6));
            ctx.closePath();
            ctx.fill();
        }
        ...
        function drawGrid() {
            ...
            // Use actual document size instead of A4 assumptions
            let widthMm = pdfWidth || 210;
            let heightMm = pdfHeight || 297;
            
            // Convert mm to canvas pixels
            let xFactor = w / widthMm;
            let yFactor = h / heightMm;
        ...
        function canvasToMM(x, y) {
            ...
            let mmX = Math.round((x / measurementCanvas.width * widthMm) * 10) / 10;
            let mmY = Math.round((y / measurementCanvas.height * heightMm) * 10) / 10;
            return { x: mmX, y: mmY };
        }
        ...
        function mmToCanvas(x, y) {
            ...
            let canvasX = x * measurementCanvas.width / widthMm;
            let canvasY = y * measurementCanvas.height / heightMm;
        ...
        function drawMeasurement() {
            measureCtx.clearRect(0, 0, measurementCanvas.width, measurementCanvas.height);
            drawAutoMeasurements(); // Keep the auto measurements visible
